 ...
 * files (PLAN.md, .konveyor/analysis.json) the playbook stages chain
...
/** Files the stages write to the branch, by convention (ADR 0006 era). */
...
const muted: CSSProperties = { color: "var(--pf-t--global--text--color--subtle)" };
    ...
    const tick = async () => {
      ...
      try {
        ...
        const rows: CommitRow[] = data
          .filter((c): c is GitHubCommitJson & { sha: string } => typeof c.sha === "string")
          .map((c) => ({
            sha: c.sha,
            htmlUrl: c.html_url ?? "",
            message: truncate((c.commit?.message ?? "").split("\n")[0] ?? "", 80),
            date: c.commit?.author?.date ?? c.commit?.committer?.date ?? undefined,
          }));
        setCommits(rows);
        setFeed("ok");
    ...
    const links = STAGE_OUTPUT_PATHS.map((path) => {
      ...
      return (
        <span key={path} style={{ marginRight: "1rem" }}>
          ...
          {withPresence && filePresence[path] === false && <span style={muted}> not present yet</span>}
        </span>
      );
    }).filter(Boolean);
          ...
          {commits !== null &&
            ...
            commits.map((c) => (
              ...
              >
                ...
                <span style={muted}>{formatAge(c.date)}</span>
              </div>
            ))}
        </div>
      )}
